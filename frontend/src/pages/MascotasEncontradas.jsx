...
export default function MascotasEncontradas() {
    ...
    useEffect(() => {
        dashboardService.get()
            .then(res => setReportes((res.data.reportes || []).filter(r => r.tipo === 'ENCONTRADO')))
            .catch(() => setReportes([]))
    ...
    return (
        <div className="container py-4">
            ...
            <div className="d-flex align-items-center gap-3 mb-4">
                ...
                <div>
                    <h3 className="fw-bold mb-0 text-success">
                        <i className="bi bi-check-circle me-2"></i>Mascotas Encontradas
                    </h3>
                    <p className="text-muted mb-0">
                        {reportes.length} mascota{reportes.length !== 1 ? 's' : ''} reportada{reportes.length !== 1 ? 's' : ''} como encontrada
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                <div className="page-loading"><div className="spinner-border text-success" role="status"></div></div>
            ) : filtrados.length === 0 ? (
                <div className="empty-state">
                    <i className="bi bi-inbox"></i>
                    <h5>{busqueda ? 'Sin resultados para tu búsqueda' : 'No hay mascotas encontradas aún'}</h5>
                    <p>{busqueda ? 'Prueba con otro término' : 'Sé el primero en reportar una mascota encontrada'}</p>
                    {busqueda
                        ? <button className="btn btn-outline-secondary mt-2" onClick={() => setBusqueda('')}>Limpiar búsqueda</button>
                        : <Link to="/reportar" className="btn btn-success mt-2">Reportar mascota encontrada</Link>
                    }
                </div>
            ) : (
                            ...
                            <Link to={`/reporte/${reporte.id}`} className="text-decoration-none">
                                <div className="card card-reporte h-100">
                                    <div className="card-img-wrapper">
                                        ...
                                        <span className="badge badge-encontrado position-absolute" style={{ top: '12px', left: '12px' }}>
                                            <i className="bi bi-check-circle me-1"></i>ENCONTRADO
                                        </span>
                                    </div>
